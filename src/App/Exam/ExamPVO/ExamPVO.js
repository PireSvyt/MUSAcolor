...
export default function ExamPVO(props) {
    ...
    function logOutcome() {
      ...
      Object.keys(outcome.rows).map((row) => {
        Object.keys(outcome.rows[row].cols).map((col) => {
          if (
            Object.keys(colorCount).includes(outcome.rows[row].cols[col].color)
          ) {
  ...
  let changes = {
    ...
    flip: (c) => {
      let currentInputs = { ...inputs }
      if (
        currentInputs.rows[c.row].invalid === true
  ...
  const [chartInputs, setchartInputs] = useState({
    options: {
      ...
      labels: testColors.map((testColor) => {
        return testColor.name
      }),
      colors: testColors.map((testColor) => {
      ...
      testColors.map((testColor) => {
        newChartInputs.series.push({
          ...
          name:
            testColor.name +
            ' / ' +
            props.exam.analysis.colors[testColor.color],
        })
        id += 1
                  ...
                  >
                    ...
                    {Object.keys(inputs.rows[row].cols).map((col) => {
                      ...
                      return (
                        <Box
                          key={'row-' + row + '-col-' + col}
                          data-testid={
                            'component-pvoexam-test-' + 'R-' + row + '-C-' + col
                          }
                          sx={{
                            m: m + 'px',
                            height: tileSize,
                    ...
                    >
                      <CheckCircleIcon
                        ...
                        data-testid={
                          'component-pvoexam-test-invalid-' + 'R-' + row
                        }
                      />
                    </Box>
                  </Box>
