...
import { Box, Stack, Typography, List, ListItem, Paper } from '@mui/material'
import LinearProgress from '@mui/material/LinearProgress'

import { serviceAdminGetDatabaseLoad } from '../admin.services.js'
  ...
  return (
    <Box sx={{ width: '100%', pt: 0, pb: 0, pl: 0.5 }}>
      <Stack
        ...
        sx={{ width: '100%' }}
      >
        <Typography>{props.stat.name}</Typography>
        <Typography>
          {props.stat.count +
            ' / ' +
            Math.floor(props.stat.size) +
            'Kb / ' +
            props.stat.weight +
            '%'}
        </Typography>
